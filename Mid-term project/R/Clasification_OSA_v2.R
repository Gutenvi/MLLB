...
df_OSA_male <- subset(df_OSA, df_OSA$Gender == 2)
...
df_OSA_male$EF <- df_OSA_male$BMI * df_OSA_male$Cervical

glm.fit=glm(OSA~BMI+Age+Cervical+Gender,data=df_OSA_male,
            family=binomial)
...
#     (think on the decision threshold, ROC, AUC curves, etc)
...
set.seed(123)
train = sample(412,300)

df_OSA_male_train=df_OSA_male[train,]
df_OSA_male_test=df_OSA_male[-train,]


glm.fit=glm(OSA~BMI+Age+Cervical+Gender,data=df_OSA_male_train,
            family=binomial)
...
# train the model
model <- train(OSA~BMI+Age+Cervical+Gender, data=df_OSA_male,
...
#Confusion Matrix
predictions <- predict(model, df_OSA_male)
# summarize results
confusionMatrix(predictions, df_OSA_male$OSA)
...
library(pROC)
glmPredict <- predict(model,newdata = df_OSA_male , type="prob")
glmROC <- roc(df_OSA_male$OSA,glmPredict[,"Severe"])
glmROC
ggroc(glmROC, alpha = 0.5, colour = "red", linetype = 1, size = 2)+
  scale_color_discrete(guide = guide_legend(title = "method"))
...
knn.model <- train(OSA~BMI+Age+Cervical+Gender,
                   data=df_OSA_male,
                   method = 'knn',
                   tuneGrid = k_values,
                   trControl = train_control)
library(pROC)
knnPredict <- predict(knn.model,newdata = knn.model$trainingData , type="prob")
knnROC <- roc(df_OSA_male$OSA,knnPredict[,"Severe"])
knnROC
ggroc(knnROC, alpha = 0.5, colour = "red", linetype = 1, size = 2)+
  scale_color_discrete(guide = guide_legend(title = "method"))

#Confusion Matrix
predictions <- predict(knn.model, df_OSA_male)
# summarize results
confusionMatrix(predictions, df_OSA_male$OSA)
...
# define training control
RF_train_control <- trainControl(method="repeatedcv", number=10, repeats=3,
...
randomForest.model$finalModel$predicted

library(pROC)
rfPredict <- predict(randomForest.model$finalModel,newdata = df_OSA_male , type="prob")
rfROC = roc(as.numeric(randomForest.model$trainingData$.outcome=='Severe'),aggregate(Severe~rowIndex,randomForest.model$pred,mean)[,'Severe'])
#rfROC <- roc(df_OSA_male$OSA,rf.pred)
rfROC
ggroc(rfROC, alpha = 0.5, colour = "red", linetype = 1, size = 2)+
  scale_color_discrete(guide = guide_legend(title = "method"))
#Confusion Matrix
predictions <- randomForest.model$finalModel$predicted
# summarize results
confusionMatrix(predictions, factor(df_OSA_male$OSA))
...
tune_grid <- expand.grid(nrounds = 200,
                         ...
                         min_child_weight = 0,
                         subsample = 0.5)
xgb.model <- train(OSA~BMI+Age+Cervical+Gender, data = df_OSA_male, method = "xgbTree",
                trControl=RF_train_control,
                tuneGrid = tune_grid,
                tuneLength = 10)
library(pROC)
xgbPredict <- predict(xgb.model,newdata = df_OSA_male , type="prob")
xgbROC = roc(as.numeric(xgb.model$trainingData$.outcome=='Severe'),aggregate(Severe~rowIndex,xgb.model$pred,mean)[,'Severe'])
ggroc(xgbROC, alpha = 0.5, colour = "red", linetype = 1, size = 2)+
  scale_color_discrete(guide = guide_legend(title = "method"))
#Confusion Matrix
xgb.pred <- aggregate(Severe~rowIndex,xgb.model$pred,mean)[,'Severe']
predictions <- rep("Healthy",412)
predictions[xgb.pred>.5]="Severe"
predictions <-factor(predictions)
confusionMatrix(predictions, factor(df_OSA$OSA))
